...
export function TicketFormStep1({
  control,
  errors,
  categoriesData,
}: TicketFormStep1Props) {
  const [selectedMainIssue, setSelectedMainIssue] = useState("");
  const [availableSubIssues, setAvailableSubIssues] = useState<
    Record<string, any>
  >({});
        ...
        <CardContent className="space-y-4">
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    <SelectContent>
                      {}
                      {Object.entries(availableSubIssues).map(
                        ([key, subIssue]: [string, any]) => (
                          <SelectItem key={key} value={key}>
                            <span className="text-right">
                              {subIssue.label || subIssue}
                            </span>
                          </SelectItem>
                        )
                      )}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <p className="text-sm text-blue-800 text-right">
              <strong>راهنما:</strong> ابتدا دسته اصلی مشکل خود را انتخاب کنید،
              سپس از فهرست دوم مشکل دقیق خود را مشخص کنید. این کار به ما کمک
              می‌کند تا بهترین راه‌حل را برای شما پیدا کنیم.
            </p>
          </div>
        </CardContent>
      </Card>
