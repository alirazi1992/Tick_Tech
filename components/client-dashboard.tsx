...
interface ClientDashboardProps {
  ...
  onSectionChange?: (section: "tickets" | "create") => void;
}
...
export function ClientDashboard({
  ...
  const [statusSummaryDialog, setStatusSummaryDialog] =
    useState<SummaryFilter | null>(null);

  useEffect(() => {
    if (activeSection === "create" && !ticketDialogOpen) {
      setTicketDialogOpen(true);
    }
    if (activeSection !== "create" && ticketDialogOpen) {
      setTicketDialogOpen(false);
    }
  }, [activeSection, ticketDialogOpen]);
  ...
  const summaryDialogTitles: Record<SummaryFilter, string> = {
    all: "همه تیکت‌ها",
    open: "در انتظار پاسخ",
    "in-progress": "در حال انجام",
    resolved: "حل شده",
  ...
  const summaryCardInteractiveClasses =
    "cursor-pointer transition hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary";

  const handleSummaryCardClick = (status: SummaryFilter) => {
    setStatusSummaryDialog(status);
  };

  const summaryDialogTickets =
    statusSummaryDialog === null
      ? []
      : statusSummaryDialog === "all"
      ? userTickets
      : userTickets.filter((ticket) => ticket.status === statusSummaryDialog);

  const summaryDialogTitle =
    statusSummaryDialog === null
      ? ""
      : summaryDialogTitles[statusSummaryDialog];

  const summaryDialogBadgeClass =
    statusSummaryDialog && statusSummaryDialog !== "all"
      ? statusColors[statusSummaryDialog]
  ...
  const renderTicketRow = (
    ...
    const idStr = String(ticket.id ?? "");
    const rowKey = idStr || String(ticket.createdAt);
    ...
    return (
      <TableRow
        ...
        className={
          clickable
            ? "cursor-pointer hover:bg-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-primary"
            : undefined
        }
      >
        <TableCell>
    ...
    <div className="space-y-6 font-iran" dir="rtl">
      <div className="flex justify-between items-center">
        ...
        <Dialog
          ...
          onOpenChange={(open) => {
            ...
            onSectionChange?.(open ? "create" : "tickets");
          }}
        >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card
          ...
          className={summaryCardInteractiveClasses}
        >
        ...
        <Card
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              handleSummaryCardClick("in-progress");
            }
          }}
          className={summaryCardInteractiveClasses}
        >
        ...
        <Card
          ...
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              handleSummaryCardClick("resolved");
            }
          }}
          className={summaryCardInteractiveClasses}
        >
            ...
            <Table>
              ...
              <TableBody>
                {filteredTickets.length > 0 ? (
                  filteredTickets.map((ticket) => renderTicketRow(ticket))
                ) : (
                  <TableRow>
      ...
      {/* Summary dialog */}
      <Dialog
        open={statusSummaryDialog !== null}
        onOpenChange={(open) => {
          if (!open) setStatusSummaryDialog(null);
        }}
      >
        <DialogContent
          ...
          {statusSummaryDialog && (
            <div className="space-y-4">
              <div className="flex justify-end">
                <Badge className={`${summaryDialogBadgeClass} font-iran`}>
                  {summaryDialogTitle}
                </Badge>
              </div>
              ...
              <div className="border rounded-lg overflow-hidden">
                <Table>
                  ...
                  <TableBody>
                    {summaryDialogTickets.length > 0 ? (
                      summaryDialogTickets.map((ticket) =>
                        renderTicketRow(ticket, { clickable: true })
                      )
                    ) : (
                      <TableRow>
                        <TableCell colSpan={8} className="text-center py-8">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="font-iran">هیچ تیکتی یافت نشد</p>
                          </div>
                        </TableCell>
                      </TableRow>
                    )}
